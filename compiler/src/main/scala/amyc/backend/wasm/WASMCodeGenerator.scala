...
object WASMCodeGenerator extends Pipeline[Program, Module] {
  ...
  private def cgModule(moduleDef: ModuleDef)(using Context): List[Function] = {
    ...
    defs.collect {
      ...
      optExpr.toList.map { expr =>
        val mainFd = FunDef(
          FunctionSymbol(Identifier.fresh("main"), name.asInstanceOf),
          Nil,
  ...
  def cgFunction(fd: FunDef, owner: Identifier, isMain: Boolean)(using
      Context
  ): Function = {
    // Note: We create the wasm function name from a combination of
    // module and function name, since we put everything in the same wasm module.
    val sig = symbols
      .getFunction(owner.name, fd.name.name)
      .map(_.signature.idx)
      .getOrElse(0)
    Function(fd, owner, isMain, sig) {
      val body = cgExpr(fd.body)
      withComment(fd.toString) {
        if isMain then body <:> drop // Main functions do not return a value,
  ...
  def cgExpr(expr: Expr)(using LocalsHandler, Context): Code = {
    expr match {
      ...
      case InfixCall(_, op, _) =>
        reporter.fatal(
          s"Cannot generate wasm code for operator, should not appear here $op"
        )
      case Not(e) =>
        cgExpr(e) <:> i32.eqz
      case Neg(e) =>
      ...
      case UseStatement(use) =>
        ...
        Comment(s"cannot generate wasm code for use statement $use") <:> i32
          .const(0)
      case _ =>
        ctx.reporter.fatal(
          s"Cannot generate wasm code for $expr",
          expr.position
        )
    }
  }
  ...
  def genFunctionCall(args: List[Expr], qname: Symbol)(using
      LocalsHandler,
      Context
  ) =
    ...
    else if qname == stdDef.binop_&& then and(cgExpr(args(0)), cgExpr(args(1)))
    else if qname == stdDef.binop_|| then or(cgExpr(args(0)), cgExpr(args(1)))
    else if qname == stdDef.binop_== then equ(cgExpr(args(0)), cgExpr(args(1)))
    else if qname == stdDef.binop_++ then
  ...
  def genConstructorCall(sym: ConstructorSymbol, args: List[Expr])(using
      LocalsHandler,
      Context
  ) = {
    ...
    global.get(memoryBoundary) <:>
      ...
      i32.store <:> {
        ...
        yield adtField(local.get(l), idx) <:> // Compute the offset to store in
  ...
  def matchAndBind(pat: Pattern)(using LocalsHandler, Context): Code =
    pat match {
  ...
  /**
    ...
    * @return
    */
  def genLiteralPattern(lit: Literal[_])(using LocalsHandler)(using Context) =
    cgExpr(lit) <:> i32.eq
  ...
  /**
    ...
    * @return
    */
  def genCaseClassPattern(constr: QualifiedName, args: List[Pattern])(using
      LocalsHandler
  )(using Context) = {
    val idx = lh.getFreshLocal
    val code = args.map(matchAndBind).zipWithIndex.map { p =>
      adtField(local.get(idx), p._2) <:> i32.load <:> p._1
    }
      ...
      ift(
        {
          ...
          equ(
            loadLocal(idx),
            constructor(constr.asInstanceOf[ConstructorSymbol].signature)
          )
        }, {
          ...
          mkBoolean(
            false
          ) // HR : Signal that the scrut does not follow this pattern
        }
      )
  }
