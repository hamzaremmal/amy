...
object Function {
  ...
  def apply(fd: FunDef, owner: Identifier, isMain: Boolean, idx: Int)(
      codeGen: LocalsHandler ?=> Code
  ): Function =
    given LocalsHandler =
      new LocalsHandler(fd.params.map(_.name.id), textmode = false)
  ...
  def apply(name: String, args: Int, isMain: Boolean, idx: Int)(
      codeGen: LocalsHandler ?=> Code
  ): Function =
