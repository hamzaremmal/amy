...
object WASMFileGenerator:
  ...
  def apply(m: Module)(using Context): String =
    ...
    val w2wOptions =
      s"${pathWithExt(m, wat_ext)} -o ${pathWithExt(m, wasm_ext)}"

    try
      try s"$local $w2wOptions".!!
      catch
        case _: IOException =>
          s"$inPath $w2wOptions".!!
    catch
      ...
      case _: RuntimeException =>
        reporter.fatal(
          s"wat2wasm failed to translate WebAssembly text file ${pathWithExt(m, wat_ext)} to binary"
        )
