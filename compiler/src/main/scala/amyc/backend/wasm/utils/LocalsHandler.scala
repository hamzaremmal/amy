...
final class LocalsHandler(val params: Int, textmode: Boolean = false):
  ...
  private val locals_ =
    mutable.HashMap.empty[Identifier, localidx].withDefaultValue(-1)
  ...
  def this(args: List[Identifier], textmode: Boolean) =
  ...
  def apply(id: Identifier)(using Context): localidx =
    // locals_.getOrElse(id, {
    //  reporter.fatal(s"Cannot find $id in the LocalsHandler")
    // })
    locals_(id)

  def getFreshLocal(i: Identifier): localidx =
    locals_.getOrElseUpdate(
      i, {
