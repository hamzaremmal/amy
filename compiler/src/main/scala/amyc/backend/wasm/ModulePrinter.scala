...
object ModulePrinter {
  implicit private def s2d(s: String): Raw = Raw(s)
  ...
  def mkTable(table: Table): Document =
    val elem: List[Document] =
      (for f <- table.elems yield Indented(s"$$${f.name} ")) ::: Raw(")") :: Nil
    val header =
      Stacked(s"(table ${table.size} funcref)", "(elem (i32.const 0)")
    Stacked {
      header :: elem
    }
  ...
  private def mkInstr(instr: Instruction): Document = {
    instr match {
      ...
      case `if`(label, tpe) =>
        Lined(List(s"if", tpe.map(mkResult).getOrElse("")), " ")
      ...
      case call_indirect(tpe, idx) =>
        Lined(List(s"call_indirect", s"$idx", mkTypeUse(tpe)), " ")
